...
and~\ref{tab:backgrounddatasets8TeV} %in Appendix~\ref{as:datasets} list the background simulation samples
...
$\roots=8\TeV$ in this analysis to allow comparison of results. %However, the trigger efficiency at 7\TeV must
% be well modelled, as described in Section~\ref{ss:trigger_ID_isolation_corrections}.
...
% The whole point is to try to choose requirements which will not affect the shape of the distribution.
% Changing btag requirement, to first order, won't change shape, because with high energy jet from QCD, some
% fraction of time, get gluon splitting to b b-bar, but that doesn't depend on the scale. So a QCD produced jet
% has a fixed probability of having a particular flavour content, i.e. bquark or a cquark in it. Every QCD
% jet has a fixed probability of having a photon conversion in it. These cuts are chosen as they are independent
% of everything else, designed to enhance the QCD background, but without changing the shape of the
% distributions.
