...
and muon+jets channels, with respect to global event variables using 5.0\fbinv of proton-proton collision data
at \roots=7\TeV and with 19.7\fbinv of data at \roots=8\TeV collected with the CMS experiment.
...
a \ttbar signal sample, and single top, \W/\ZpJets and QCD multi-jet backgrounds, a fit of the simluated
samples to data is carried out in each bin and in each channel to obtain a \ttbar event yield. The number of
signal events is then unfolded to remove detector and selection effects. Combining the result from the two
channels, the calculation of the normalised differential cross sections is performed. Comparison with common
Monte Carlo generators, including with different modelling parameters, shows broad consistency with the
Standard Model, with the exception of an observed softer transverse momentum distribution than in simulations.
